<?php 
	...
	$dis_query="select * from student_login";
	...
	$c=count($rs); 
	 ...
	 if($_GET['id']!="")
	 { 
	 $del_que= "delete from student_login where s_id='".$_GET['id']."'";	
	 mysql_query($del_que);
			...
			<td id="mainbg" valign="top">
				<div id="haupttext">
					...
					<div class="table-style">
						...
						<td><?php echo $rs[$i][3]?></td>
						<td><?php echo $rs[$i][4]?></td>
						<td><?php echo $rs[$i][5]?></td>
						<td><?php echo $rs[$i][6]?></td>
						<td><?php echo $rs[$i][7]?></td>
						<td><?php echo $rs[$i][8] . "/" . $rs[$i][9] . "/" . $rs[$i][10]; ?></td>
						<td><a href="std_update.php?id=<?php if(isset($rs[$i][0])) { echo $rs[$i][0]; } ?>">Update </a>
						<td><a href="std_dis.php?id=<?php if(isset($rs[$i][0])) { echo $rs[$i][0]; } ?>" onClick="return confirm('Are you sure you want to delete?')">delete </a>
						<?php 
							}
						?>
						</table>
					</div>
					<a href="std_add.php">Add Student</a>
					<!--End of Text-->
				</div>
			</td>
		</tr>
